...
namespace MAPapp
{
    class GetFromDatabase
    {
        ...
        private static String getJsonData(String username, String password, String command, String token, String fName, String lName, String joincode, int projectId, int sprintID, int taskid)
        {
            string url = CreateURL(username, password, command, token, fName, lName, joincode,projectId, sprintID, taskid);
        ...
        {
           // s.Start();
            String z = JsonConvert.DeserializeObject<String>(getJsonData(userName, password, "signIn", null, null, null, null, 0,0,0));
           
            return z;
        }
        public static object CreateUser(String userName, String password, String fName, String lName, String joincode)
        {
            string s = getJsonData(userName, password, "createNewUser", null, fName, lName, joincode,0,0,0);
            return JsonConvert.DeserializeObject<String>(s);
        }
        public static object GetInformation(String userName, String token)
        {
            string s = getJsonData(userName,null,"getInformation",token,null,null,null,0,0,0);
        ...
        {
            string s = getJsonData(userName, null, "getTasks", token, null, null, null, projectId, 0,0);
            try
            {
        ...
        {
            return JsonConvert.DeserializeObject<string>(getJsonData(userName, null, "joinProject", token, null, null, null, projectID, 0,0));

        }
        public static object JoinTask(String userName, String token,int taskid,int projectid)
        {
            return JsonConvert.DeserializeObject<String>(getJsonData(userName,null, "assingTask", token,null,null,null,projectid,0,taskid));
        }
